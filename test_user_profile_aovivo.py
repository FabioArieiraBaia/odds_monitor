...
async def main():
    ...
    print(f"Usando perfil real: {user_data_dir}")
    ...
    async with async_playwright() as pw:
        try:
            ...
            # Clicar no botão Ao-Vivo usando JS no topo
            ...
            if clicked:
                print("Clique no botão Ao-Vivo efetuado via JS.")
                await asyncio.sleep(8)
            else:
                print("Botão Ao-Vivo no topo não encontrado.")
                
            print(f"URL final: {page.url}")
            
            # Tirar screenshot
            screenshot_path = os.path.join(ARTIFACT_DIR, "real_profile_aovivo.png")
