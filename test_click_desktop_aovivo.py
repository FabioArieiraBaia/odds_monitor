...
async def main():
    ...
    async with async_playwright() as pw:
        try:
            ...
            # Clicar no link 'Ao-Vivo' no menu superior
            # Vamos procurar no header. No desktop, ele fica no topo.
            print("Clicando no botão 'Ao-Vivo' do header...")
            # O link no header geralmente tem classes como "hm-HeaderMenuItem"
            ao_vivo_header = page.locator("div[class*='Header'] >> text=Ao-Vivo, a[class*='Header'] >> text=Ao-Vivo, [class*='header'] >> text=Ao-Vivo, text=Ao-Vivo").first
            if await ao_vivo_header.count() > 0:
                print("Elemento Ao-Vivo no header encontrado. Clicando...")
                await ao_vivo_header.click()
                await asyncio.sleep(8)
            else:
                print("Não foi possível localizar o botão Ao-Vivo no header.")
                
            print(f"URL final: {page.url}")
            
            # Tirar screenshot
            screenshot_path = os.path.join(ARTIFACT_DIR, "desktop_ao_vivo.png")
            await page.screenshot(path=screenshot_path)
            print(f"Screenshot salvo em {screenshot_path}")
            
            fixtures_count = await page.locator(".ovm-Fixture, .ipe-EventViewDetail").count()
            print(f"Total de fixtures na tela Ao-Vivo Desktop: {fixtures_count}")
